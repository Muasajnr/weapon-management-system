...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">Edit Senjata Api</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="<?=route_to('firearms')?>">Data Senjata Api</a></li>
                    <li class="breadcrumb-item active">Edit Senjata Api</li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <!-- /.card-header -->
                    <div class="card-body">
                        <form id="form-edit-firearm">
                            <div class="form-group">
                                <label>Jenis Inventaris</label>
                                <select id="select2-data-inventory-types" name="inventory_type" class="form-control select2" style="width: 100%;">
                                    <option value="<?=$inventory_type_id?>" selected><?=$inventory_type?></option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label>Jenis Senjata</label>
                                <select id="select2-data-firearms-types" name="firearm_type" class="form-control select2" style="width: 100%;">
                                    <option value="<?=$firearm_type_id?>" selected><?=$firearm_type?></option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label>Merk Senjata</label>
                                <select id="select2-data-firearms-brands" name="firearm_brand" class="form-control select2" style="width: 100%;">
                                    <option value="<?=$firearm_brand_id?>" selected><?=$firearm_brand?></option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="firearm_number">Nomor Senjata</label>
                                <input type="text" name="firearm_number" class="form-control" id="firearm_number" value="<?=$firearm_number?>" placeholder="Masukkan Nomor Senjata...">
                            </div>
                            <div class="form-group">
                                <label for="bpsa_number">Nomor BPSA</label>
                                <input type="text" name="bpsa_number" class="form-control" id="bpsa_number" value="<?=$bpsa_number?>" placeholder="Masukkan Nomor BPSA...">
                            </div>
                            <div class="form-group">
                                <label for="condition">Kondisi</label>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="condition" value="normal" <?=($condition == 'normal' ? 'checked' : '')?>>
                                    <label class="form-check-label">Normal</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="condition" value="damaged" <?=($condition == 'damaged' ? 'checked' : '')?>>
                                    <label class="form-check-label">Rusak</label>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="desc">Deskripsi</label>
                                <textarea id="desc" name="desc" class="form-control" rows="3" placeholder="Masukkan Deskripsi..."><?=$description?></textarea>
                            </div>
...
$(function() {
    ...
    $('#form-edit-firearm').validate({
        submitHandler: function(form, event) {
            ...
            const editFirearmUrl = '<?=site_url('api/dashboard/firearms/'.$firearm_id.'/update')?>';

            $.ajax({
                type: 'PUT',
                url: editFirearmUrl,
                dataType: 'json',
                data: JSON.stringify(editedFirearm),
